...
- **Scheduled job:** When `news.daily-briefing.enabled` is true, the scheduler runs analyze for one calendar day in the configured zone and upserts with `lastSource = CRON`. The window is `[targetDate, targetDate + 1)` in that zone—use the same values in `GET .../window` to fetch that row.
